...
# ================================================================
# SIDEBAR — ORIGINAL CONTROL & FILTER
# ================================================================
...
try:
    st.sidebar.download_button(
        "📥 Download Template Excel Standard", generate_excel_template(),
...
_df = df_raw.copy()
if sel_prov:
    _df = _df[_df["Provinsi"].astype(str).isin(sel_prov)]
else:
    _df = _df.iloc[0:0]
if sel_kab:
    _df = _df[_df["Kabupaten"].astype(str).isin(sel_kab)]
else:
    _df = _df.iloc[0:0]
...
for level in sel_tingkat_diag:
    ...
    if col in _df.columns:
        mask |= _df[col].notna() & (_df[col].astype(str) != "Bukan")
...
if sel_disease != "Semua Penyakit":
    ...
    for col in level_cols.values():
        if col in df_active.columns:
            dmask |= df_active[col].astype(str).eq(sel_disease)
...
# ================================================================
# HELPERS FOR TAB 1 — keep risk stratification API compatible
# ================================================================
def build_risk_table(df):
    ...
    if "Is_Meninggal" in work.columns:
        work["_death"] = pd.to_numeric(work["Is_Meninggal"], errors="coerce").fillna(0)
    else:
        work["_death"] = work.get("Status Penderita", "").astype(str).str.lower().eq("meninggal").astype(int)
...
# ================================================================
# MAIN NAVIGATION — ORIGINAL 5 ANALYSES + ML LAYER
# ================================================================
...
# ================================================================
# TAB 1 — SITUATIONAL INTELLIGENCE
# ================================================================
with tab1:
    ...
    if df_active.empty:
        ...
        vulnerable = identifikasi_vulnerable_profile(df_active)
        risk_df = build_risk_table(df_active)
        ...
        if not risk_df.empty:
            st.dataframe(risk_df.sort_values("Risk_Score", ascending=False), use_container_width=True, hide_index=True)
        else:
        ...
        if ews_score is not None:
            a, b, c = st.columns(3)
            a.metric("EWS Score", f"{ews_score:.1f}/100")
            b.metric("Trend 7 hari", f"{trend_pct:+.1f}%")
            c.metric("Level", ews_level)
            st.caption("EWS merupakan skor rule-based untuk sinyal perubahan tren; bukan probabilitas KLB terkalibrasi.")
        else:
            st.info("Minimal 14 hari observasi diperlukan untuk EWS.")

        st.markdown("### 🗺️ Regional CFR & Outlier")
        if "Is_Meninggal" in df_active.columns:
            reg = df_active.groupby(["Provinsi", "Kabupaten"]).agg(
                Total=("Is_Meninggal", "size"), Meninggal=("Is_Meninggal", "sum")
            ).reset_index()
            reg["CFR (%)"] = (reg["Meninggal"] / reg["Total"] * 100).round(2)
            st.dataframe(reg.sort_values("CFR (%)", ascending=False).head(20), use_container_width=True, hide_index=True)

        st.markdown("### 👤 Profil Demografis")
        d1, d2, d3 = st.columns(3)
        d1.metric("Rata-rata Umur", f"{pd.to_numeric(df_active['Umur'], errors='coerce').mean():.1f} tahun")
        d2.metric("Median Umur", f"{pd.to_numeric(df_active['Umur'], errors='coerce').median():.1f} tahun")
        mode_gender = df_active["Jenis Kelamin"].mode()
        d3.metric("Jenis Kelamin Terbanyak", str(mode_gender.iloc[0]) if not mode_gender.empty else "N/A")

        st.markdown("### 🤖 Strategic AI Recommendations")
        recs = []
        if ews_score is not None and ews_score >= 60:
            recs.append("Prioritaskan verifikasi lapangan dan penguatan surveilans pada sinyal tren tinggi.")
        if not risk_df.empty and (risk_df["Risk_Level"] == "HIGH").any():
            recs.append("Terdapat wilayah dengan skor risiko relatif tinggi; lakukan investigasi epidemiologis terarah.")
        if vulnerable:
            recs.append("Fokuskan intervensi preventif pada profil populasi rentan yang teridentifikasi.")
...
with tab2:
    ...
    if df_active.empty:
        ...
        st.markdown("#### Place — Distribusi Wilayah")
        place = df_active.groupby(["Provinsi", "Kabupaten", "Kecamatan", "Desa/Kelurahan"]).size().reset_index(name="Jumlah Kasus")
        st.dataframe(place.sort_values("Jumlah Kasus", ascending=False), use_container_width=True, hide_index=True)
        st.markdown("#### Person — Demografi")
        c1, c2 = st.columns(2)
        with c1:
            age = pd.to_numeric(df_active["Umur"], errors="coerce").dropna()
            fig, ax = plt.subplots(figsize=(8, 4)); ax.hist(age, bins=15); ax.set_xlabel("Umur"); ax.set_ylabel("Jumlah Kasus"); ax.grid(alpha=.3); st.pyplot(fig)
        with c2:
        ...
        st.markdown("#### Time — Distribusi Kasus")
        epi = build_epi(df_active)
        if not epi.empty:
            st.line_chart(epi.set_index("Tanggal Sakit")["Jumlah Kasus"])
        st.markdown("#### Diagnosis")
        rows = []
        for label, col in [("Suspek", "Diagnosis Suspek"), ("Probabel", "Diagnosis Probabel"), ("Konfirm", "Diagnosis Konfirm")]:
            if col in df_active.columns:
                rows.append({"Tingkat Diagnosis": label, "Jumlah Kasus": int((df_active[col].astype(str) != "Bukan").sum())})
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

# ================================================================
# TAB 3 — EPIDEMIC CURVE & FORECAST
# ================================================================
with tab3:
    ...
    if epi.empty:
        ...
        if len(epi) >= 7:
            shape, desc, interp, pred, metrics = deteksi_bentuk_kurva(epi, sel_disease)
            st.info(f"**Bentuk kurva:** {shape}\n\n{desc}")
            with st.expander("Interpretasi & prediksi"):
                st.write(interp); st.write(pred); st.json(metrics)
        forecast = prediksi_kurva_holt_winters(epi, 14)
        if isinstance(forecast, dict):
            fig, ax = plt.subplots(figsize=(14, 5))
            ax.bar(epi["Tanggal Sakit"], epi["Jumlah Kasus"], alpha=.7, label="Historis")
            ax.plot(forecast["dates"], forecast["forecast"], "--o", label="Holt-Winters 14 Hari")
            ax.fill_between(forecast["dates"], forecast["lower"], forecast["upper"], alpha=.15, label="Interval")
            ax.legend(); ax.grid(alpha=.3); st.pyplot(fig)
            a, b, c = st.columns(3)
            a.metric("Tren", forecast["trend"])
            b.metric("Prediksi Puncak", forecast["peak_date"].strftime("%d %b %Y"))
            c.metric("Kasus Puncak", f"{forecast['peak_value']:.0f}")
        elif forecast == "KURANG_DATA":
            st.info("Data historis kurang untuk forecast 14 hari.")
        rt = hitung_effective_rt(epi)
        if rt:
            st.metric("Rₜ rata-rata", f"{rt['rt_avg']:.2f}")
            st.info(rt["interpretation"])
...
# ================================================================
# TAB 4 — SPATIAL + DBSCAN
# ================================================================
with tab4:
    st.markdown("### 🗺️ Peta Spasial & AI DBSCAN")
    if df_active.empty:
        st.warning("Tidak ada data aktif.")
    else:
        ...
        if geo.empty:
            ...
            if st.button("📍 Lengkapi koordinat otomatis"):
                df_active = lengkapi_koordinat_otomatis(df_active)
                geo = df_active.dropna(subset=["Latitude", "Longitude"]).copy()
        if not geo.empty:
            from sklearn.cluster import DBSCAN
            coords = np.radians(geo[["Latitude", "Longitude"]].astype(float))
            labels = DBSCAN(eps=3.0/6371.0088, min_samples=4, metric="haversine").fit(coords).labels_
            geo["Cluster"] = labels
            st.dataframe(geo[["Provinsi", "Kabupaten", "Kecamatan", "Desa/Kelurahan", "Latitude", "Longitude", "Cluster"]].head(500), use_container_width=True, hide_index=True)
            center = [float(geo["Latitude"].mean()), float(geo["Longitude"].mean())]
            m = folium.Map(location=center, zoom_start=8)
            for _, r in geo.head(500).iterrows():
                folium.CircleMarker([r["Latitude"], r["Longitude"]], radius=4, popup=f"{r['Desa/Kelurahan']} | Cluster {r['Cluster']}").add_to(m)
    ...
    if df_active.empty:
        ...
        try:
            ...
            if isinstance(result, pd.DataFrame):
                ...
                st.info("Hasil analisis faktor risiko belum tersedia dalam format tabel.")
        except Exception as exc:
            st.warning(f"Analisis bivariat tidak dapat dijalankan pada filter ini: {exc}")

# ================================================================
# TAB 6 — ML INTELLIGENCE ENGINE
# ================================================================
with tab6:
    ...
    with ml1:
        ...
        if st.button("Train Severity Model", key="train_severity"):
            res = train_case_severity(df_active)
            st.session_state["severity_model"] = res if res.get("status") == "ok" else None
            st.session_state["severity_result"] = res
        res = st.session_state.get("severity_result")
        if res:
            if res.get("status") == "ok":
                st.json(res["metrics"])
                st.dataframe(res["feature_importance"], use_container_width=True, hide_index=True)
        ...
        st.caption("Target severity pada demo diturunkan dari mortalitas/rawat inap; validasi klinis diperlukan untuk penggunaan nyata.")

    with ml2:
        st.markdown("#### KLB / Outbreak 7-Day Prediction")
        if st.button("Train KLB Model", key="train_klb"):
            st.session_state["klb_result"] = train_klb_prediction(df_active)
        res = st.session_state.get("klb_result")
        if res:
            if res.get("status") == "ok":
                st.json(res["metrics"]); st.caption(f"Derived demo threshold: {res.get('derived_threshold')}")
                pred = predict_klb(df_active, res["model"])
        ...
        st.caption("Label KLB pada mode demo diturunkan dari total kasus 7 hari berikutnya; definisi KLB produksi harus memakai outcome tervalidasi.")

    with ml3:
        st.markdown("#### Spatial Outbreak Prediction")
        if st.button("Train Spatial Model", key="train_spatial"):
            st.session_state["spatial_result"] = train_spatial_outbreak(df_active)
        res = st.session_state.get("spatial_result")
        if res:
            if res.get("status") == "ok":
                st.json(res["metrics"])
                pred = predict_spatial_outbreak(df_active, res["model"])
    ...
    with ml4:
        ...
        if st.button("Train Vulnerable Model", key="train_vulnerable"):
            st.session_state["vulnerable_result"] = train_vulnerable_population(df_active)
        res = st.session_state.get("vulnerable_result")
        if res:
            if res.get("status") == "ok":
                st.json(res["metrics"])
                pred = predict_vulnerable_population(df_active, res["model"])
    ...
    with ml5:
        ...
        epi = build_epi(df_active)
        if st.button("Run Robust Forecast", key="run_forecast"):
            st.session_state["forecast_result"] = robust_forecast(epi, 14)
        res = st.session_state.get("forecast_result")
        if res:
            if res.get("status") == "ok":
                a, b, c = st.columns(3)
                a.metric("Backtest MAE", f"{res['mae']:.2f}")
                b.metric("Backtest RMSE", f"{res['rmse']:.2f}")
                c.metric("Prediksi Puncak", f"{res['peak_value']:.1f}")
                st.write("Model weights:", res["weights"])
                fig, ax = plt.subplots(figsize=(14, 5)); ax.plot(res["dates"], res["forecast"], "-o", label="Ensemble forecast"); ax.fill_between(res["dates"], res["lower"], res["upper"], alpha=.15); ax.grid(alpha=.3); ax.legend(); st.pyplot(fig)
            else:
                st.warning(res.get("message", "Forecast gagal."))

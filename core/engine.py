...
2. epidemiology: one disease + any geographic scope for TIME + PERSON + PLACE.
"""
...
def _disease_per_case(df: pd.DataFrame) -> pd.Series:
    ...
    for column in reversed(DISEASE_COLUMNS):
        ...
        values = df[column].astype(str).str.strip(); valid = ~values.str.lower().isin(NON_DISEASE_VALUES)
...
def _trias_summary(df: pd.DataFrame, national_disease_df: pd.DataFrame, disease: str, geographic_scope: str) -> dict[str, Any]:
    """Narrative TIME+PERSON+PLACE summary, with national denominator when scope is Indonesia."""
    total=len(df); national_total=len(national_disease_df); deaths=_death_series(df).sum(); cfr=deaths/total*100 if total else 0.0
    summary=[]
    if "Provinsi" in df.columns and total:
        g=df.groupby("Provinsi",dropna=False).agg(Kasus=("Provinsi","size"),Meninggal=("_death","sum")).reset_index()
        g["CFR"]=np.where(g["Kasus"]>0,g["Meninggal"]/g["Kasus"]*100,0)
        top_cases=g.sort_values(["Kasus","Provinsi"],ascending=[False,True]).iloc[0]
        top_cfr=g.sort_values(["CFR","Kasus"],ascending=[False,False]).iloc[0]
        denominator=national_total if geographic_scope=="Indonesia" else total
        share=top_cases["Kasus"]/denominator*100 if denominator else 0
        summary.append(f"Kasus **{disease}** terbanyak berada di **{top_cases['Provinsi']}**, sebanyak **{int(top_cases['Kasus']):,} kasus** ({share:.2f}% dari seluruh kasus {disease} di Indonesia) dengan CFR **{top_cases['CFR']:.2f}%**.")
        if str(top_cfr["Provinsi"]) != str(top_cases["Provinsi"]):
            summary.append(f"CFR tertinggi bukan berada di wilayah dengan kasus terbanyak, melainkan di **{top_cfr['Provinsi']}**, sebesar **{top_cfr['CFR']:.2f}%** ({int(top_cfr['Meninggal'])} meninggal dari {int(top_cfr['Kasus'])} kasus).")
        else:
            summary.append(f"Wilayah dengan kasus terbanyak juga memiliki CFR tertinggi, yaitu **{top_cfr['CFR']:.2f}%**.")
    if "Umur" in df.columns and total:
        ...
        ag=bins.value_counts(sort=False,dropna=True)
        if not ag.empty:
            a=ag.idxmax(); n=int(ag.max()); summary.append(f"Distribusi umur terbesar terdapat pada kelompok **{a}**, sebanyak **{n:,} kasus ({n/total*100:.2f}% dari seluruh kasus {disease} pada scope analisis)**.")
    ...
    if "Provinsi" in df.columns and total and geographic_scope=="Indonesia":
        top10=df.groupby("Provinsi",dropna=False).size().reset_index(name="Jumlah Kasus").sort_values(["Jumlah Kasus","Provinsi"],ascending=[False,True]).head(10).reset_index(drop=True)
        top10.insert(0,"NO.",np.arange(1,len(top10)+1))
    return {"narrative":" ".join(summary),"top10_province":top10,"total_scope_cases":total,"national_disease_cases":national_total,"scope":geographic_scope,"scope_cfr":round(cfr,2)}

class IntelligenceEngine:
    def prepare(self,df:pd.DataFrame,scope:QueryScope|None=None)->IntelligenceResult:
        query_scope=(scope or QueryScope()).normalized(); scoped=apply_scope(df,query_scope)
    ...
    def descriptive(self,df:pd.DataFrame)->dict[str,Any]:
        work=df.copy(deep=True); work["_disease"]=_disease_per_case(work); total=len(work); deaths=int(_death_series(work).sum())
    ...
    def analyze(self,df:pd.DataFrame,scope:QueryScope|None=None,forecast_days:int=14,include_ml:bool=False,mode:str="epidemiology")->dict[str,Any]:
        ...
        eligibility=validate_scope_for_special_analysis(work,prepared.scope.disease,geographic_level,10,14); profile=resolve_disease_profile(prepared.scope.disease)

...
class IntelligenceEngine:
    ...
    def analyze(self,df,scope=None,forecast_days=14,include_ml=False,mode="epidemiology"):
        ...
        common["klb"] = evaluate_klb(work, disease=profile.name, geographic_scope=geographic_level) if disease_intel.family=="MENULAR" else {"enabled":False,"reason":f"KLB surveillance tidak relevan untuk {disease_intel.family}."}
        ...
        work=work.copy();work["_death"]=_death_series(work);trias=analyze_trias(work);epi=trias["time"];waves=deteksi_gelombang(epi) if disease_intel.family=="MENULAR" and not epi.empty else []
        try:curve=deteksi_bentuk_kurva(epi,profile.name) if disease_intel.family=="MENULAR" and len(epi)>=7 else None

...
def _trias_summary(df,national_disease_df,disease,geographic_scope):
    ...
    if "Provinsi" in df.columns and total:
        ...
        top_cases=g.sort_values(["Kasus","Provinsi"],ascending=[False,True]).iloc[0]; top_cfr=g.sort_values(["CFR","Kasus"],ascending=[False,False]).iloc[0]
        denominator=national_total if geographic_scope=="Indonesia" else total; share=top_cases["Kasus"]/denominator*100 if denominator else 0
...
class IntelligenceEngine:
    ...
    def analyze(self,df,scope=None,forecast_days=14,include_ml=False,mode="epidemiology"):
        ...
        if not eligibility.eligible:common["message"]="Analisis epidemiologi khusus belum dijalankan karena scope/data belum memenuhi syarat.";return common
        ...
        spatial=run_dbscan(work); national_disease_df=_filter_disease(df,prepared.scope.disease); trias_summary=_trias_summary(work,national_disease_df,profile.name,geographic_level)
        # Put the narrative and Indonesia top-10 directly into the rendered PERSON block so the existing UI shows them without raw object dumps.
        person=trias["person"].copy(); person["Resume TIME + PERSON + PLACE"]=trias_summary["narrative"]

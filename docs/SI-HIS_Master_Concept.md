...
### 2.3 NutriMed MyLab as the Personal Health Journey

NutriMed MyLab is not designed as a conventional health-information or appointment application. Its strategic role is to become the user's **Personal Health Journey**: a longitudinal layer that connects health observations, clinical encounters, laboratory results, medication, nutrition, preventive programs, follow-up, and outcomes over time.

The app is intended to become the user's personal interface to the SI-HIS intelligence ecosystem:

**PERSON → PERSONAL HEALTH DATA → SI-HIS → PERSONAL INTELLIGENCE → ACTION → OUTCOME → UPDATED PERSONAL HEALTH JOURNEY**

The long-term product vision is a **virtual holographic personal health assistant**. This is a future product-development direction, not a capability claimed for the current application. The envisioned device/interface would provide natural, human-like dialogue and multimodal interaction with the NutriMed MyLab user, while remaining governed by health-data privacy, clinical safety, consent, and human oversight requirements.

The future assistant is intended to:
- understand the user's longitudinal health journey;
- converse naturally about health information and next steps;
- retrieve and explain authorized personal health information;
- remind the user about preventive and follow-up activities;
- orchestrate access to doctors, laboratories, dietitians, pharmacies and other authorized services;
- receive user-generated observations and feedback;
- return outcome information to the personal health journey.

The assistant should remain an **intelligent companion and orchestration interface**, not an autonomous replacement for a clinician.
...
## 8A. Personal Health Data Ingress — The Entry Layer to SI-HIS

A critical addition to the SI-HIS architecture is the explicit definition of **where personal health data enters the ecosystem**. The personal-health layer is not limited to one application. SI-HIS should be able to receive authorized, standardized data from multiple sources and then establish a longitudinal health journey around the person.

### 8A.1 Primary personal-health sources

1. **FKTP (Fasilitas Kesehatan Tingkat Pertama)**
   - Puskesmas
   - Klinik/FKTP
   - Praktik dokter or other applicable primary-care providers
   - registration, encounter, diagnosis, observations, treatment, prescriptions, referrals and outcomes according to the applicable interoperability use case.

2. **FKRTL (Fasilitas Kesehatan Rujukan Tingkat Lanjut)**
   - Rumah sakit and applicable referral providers
   - outpatient, emergency, inpatient, procedure, laboratory, imaging, diagnosis, medication, discharge and follow-up data.

3. **NutriMed MyLab Mobile App**
   - self-reported symptoms and observations;
   - lifestyle and behavior;
   - nutrition/food tracking;
   - medication adherence;
   - home measurements where supported;
   - appointments, consultations and follow-up;
   - patient feedback and outcomes;
   - personal health goals.

4. **Partner Patient Mobile Apps**
   - authorized external digital-health applications;
   - data generated by partner services;
   - consultation/service results;
   - laboratory or medication information;
   - patient-entered data;
   - other permitted personal-health events.

5. **Other authorized ecosystem sources**
   - laboratory systems;
   - pharmacy systems;
   - corporate MCU/K3 systems;
   - payer/claims systems;
   - environmental or One Health sources where relevant.

### 8A.2 Personal Health Data Flow
...
PERSONAL HEALTH JOURNEY       POPULATION INTELLIGENCE
        │                         │
        ▼                         ▼
NutriMed MyLab              Kemenkes / BPJS /
Personal Intelligence       Dinkes / Corporate /
                            Provider Intelligence
```

### 8A.3 Authority and provenance of data

SI-HIS should retain **data provenance** for every important health event. The source should be distinguishable, for example:
- clinical/facility-originated data;
- laboratory-originated result;
- pharmacy-originated dispensing;
- payer-originated utilization/claim;
- patient self-reported data;
- device-generated data;
- partner-app data.

This distinction is important because a patient-reported symptom and a laboratory-confirmed result have different evidentiary characteristics. The system should never silently convert self-reported information into a clinically confirmed diagnosis.

### 8A.4 Identity and interoperability

SATUSEHAT uses HL7 FHIR for health-data models and APIs. For data involving a Patient resource, the SATUSEHAT documentation describes the use of the patient's IHS number obtained through the Master Patient Index. This provides an important reference for SI-HIS identity architecture, while production implementation must follow the current SATUSEHAT integration rules and authorization model.

The SI-HIS identity layer should therefore support:
...
### 8A.5 Longitudinal Personal Health Journey

The personal journey should not be represented as a collection of disconnected visits. It should become a longitudinal timeline:
...
NutriMed MyLab is the user-facing layer that makes this journey understandable and actionable to the individual, while SI-HIS provides the intelligence and orchestration layer behind it.

### 8A.6 Future Holographic Personal Health Assistant

The future product direction is to evolve the NutriMed MyLab interface beyond a conventional mobile application into a **virtual holographic personal health assistant**.

Conceptually:

**NutriMed MyLab Mobile App → Multimodal AI Companion → Holographic Personal Assistant**

The envisioned assistant could:
- maintain contextual conversation across the user's health journey;
- explain laboratory or health information in understandable language;
- ask structured follow-up questions;
- remind and coach preventive behaviors;
- coordinate authorized services;
- receive voice/dialogue-based health observations;
- interact with other SI-HIS services.

This is a product vision and research/development direction. It should not be presented as a current medical capability. Any future deployment would require dedicated human-factors, privacy, cybersecurity, clinical-safety, accessibility and regulatory assessment.
